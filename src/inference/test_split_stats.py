...
def generate(sd, input, **kwargs) -> Image.Image:
    if len(input.shape) == 3:
        input = input.unsqueeze(0)
    if s == 0:
        return vae_decode(sd, image / 0.18215)
    else:
        return sd(
            image=input,
            **kwargs,
...
if __name__ == "__main__":
    ...
    photos = np.load(args.path / "photos_test_resized.npy", mmap_mode="r").astype(np.uint8)
    ...
    pixel_error_sum = 0
    ssim_sum = 0
    for latent, ref in tqdm(zip(p, photos)):
        img = np.asarray(vae_decode(sd, latent))
        img = torch.from_numpy(np.array(img))
        pe = torch.sum(torch.abs(img - ref))
        pixel_error_sum += pe
        s = cast(float, ssim(np.asarray(img), np.asarray(ref), channel_axis=2, data_range=255))
        ssim_sum += s
        # print(pe, s)
